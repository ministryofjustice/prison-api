...
There is an alert in Application Insights called `Elite2-api - Inactivity alert`. It fires if prison-api hasn't received any successful requests in the last 10 minutes.
...
* Offender images can be set using the endpoint PUT /api/images//offender/{offenderNo}
* This requires a token with the role ROLE_SYSTEM_USER
* The body of the request should be a base64-encoded JPEG file
* The optional query parameter ?fullSizeImage=true can be used if this is a full-sized image.
* The default assumes this is a thumbnail-sized facial image.
* If no booking is found for the offender this will fail with a 404 and appropriate message.

#### Converting and uploading an image

* Generated, generic images can be found here: https://generated.photos/faces/natural/adult/male
* Save the JPEG image as a file locally.
* Convert to base64 using `cat <filename.jpg> | base64 -w 0 > image.json`
* Using curl (the process of obtaining a token with apppropriate roles is left out here):

```
  # PUT an image for an offender
  curl -X PUT "https://<URL to prison API host>/api/images/offender/{prisonerNumber}" \
