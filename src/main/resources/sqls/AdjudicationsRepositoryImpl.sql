...
FIND_ADJUDICATION {
    ...
    ,		AL.LONG_DESCRIPTION   ESTABLISHMENT
    ...
    FROM  AGENCY_INCIDENTS AI
      INNER JOIN AGENCY_INCIDENT_PARTIES AIP ON AIP.AGENCY_INCIDENT_ID = AI.AGENCY_INCIDENT_ID
      INNER JOIN AGENCY_LOCATIONS AL ON AL.AGY_LOC_ID = AI.AGY_LOC_ID
      INNER JOIN AGENCY_INTERNAL_LOCATIONS AIL ON AIL.INTERNAL_LOCATION_ID = AI.INTERNAL_LOCATION_ID
    ...
    WHERE AIP.OFFENDER_BOOK_ID IN (
        SELECT OFFENDER_BOOK_ID FROM OFFENDER_BOOKINGS OB
          JOIN OFFENDERS OFF ON OFF.OFFENDER_ID = OB.OFFENDER_ID
          WHERE OFF.OFFENDER_ID_DISPLAY = :offenderNumber)
    AND AIP.OIC_INCIDENT_ID = :adjudicationNumber
...
FIND_HEARINGS {
    ...
    WHERE OH.OIC_INCIDENT_ID = :adjudicationNumber
}
...
FIND_SANCTIONS {
     ...
     FROM OMS_OWNER.OFFENDER_OIC_SANCTIONS SAN
     ...
     WHERE OIC_HEARING_ID IN (:hearingIds);
}