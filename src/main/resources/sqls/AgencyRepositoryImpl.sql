...
SELECT IEP_DETAILS.OFFENDER_BOOK_ID AS BOOKING_ID,
       POS_NEG_IEPS.POSITIVE_IEPS AS POSITIVE_IEPS,
       POS_NEG_IEPS.NEGATIVE_IEPS AS NEGATIVE_IEPS,
       COALESCE(PROVEN_ADJUDICATIONS_SELECT.PROVEN_ADJUDICATIONS, 0) AS PROVEN_ADJUDICATIONS,
...
FROM ( SELECT OB.OFFENDER_BOOK_ID,
       ...
       FROM OFFENDER_BOOKINGS OB
                INNER JOIN OFFENDER_IEP_LEVELS OIL ON OB.OFFENDER_BOOK_ID = OIL.OFFENDER_BOOK_ID
            ...
            (
                ...
                GROUP BY OB.OFFENDER_BOOK_ID ) GROUPED_BY_TIME
       WHERE OB.AGY_LOC_ID = :agencyId
         AND OB.BOOKING_SEQ = :bookingSeq
         AND OIL.IEP_TIME = GROUPED_BY_TIME.MAX_TIME) IEP_DETAILS
     FULL OUTER JOIN (SELECT OB.OFFENDER_BOOK_ID,
                             SUM(CASE WHEN OCN.CASE_NOTE_TYPE = 'POS'
                                 AND OCN.CASE_NOTE_SUB_TYPE = 'IEP_ENC'
                                 AND OCN.CREATE_DATETIME > :threeMonthsAgo
                      ...
                      FROM OFFENDER_BOOKINGS OB
                               JOIN OFFENDER_CASE_NOTES OCN ON OCN.OFFENDER_BOOK_ID = OB.OFFENDER_BOOK_ID
                      WHERE OB.AGY_LOC_ID = :agencyId
                        AND OB.BOOKING_SEQ = :bookingSeq
                      GROUP BY OB.OFFENDER_BOOK_ID) POS_NEG_IEPS ON POS_NEG_IEPS.OFFENDER_BOOK_ID = IEP_DETAILS.OFFENDER_BOOK_ID
         FULL OUTER JOIN (
    SELECT OB.OFFENDER_BOOK_ID,
           COUNT (DISTINCT OHR.OIC_HEARING_ID) AS PROVEN_ADJUDICATIONS
    FROM OFFENDER_OIC_SANCTIONS OOS
             JOIN OFFENDER_BOOKINGS OB ON OOS.OFFENDER_BOOK_ID = OB.OFFENDER_BOOK_ID
             JOIN OIC_HEARING_RESULTS OHR ON OOS.OIC_HEARING_ID = OHR.OIC_HEARING_ID
    WHERE OB.AGY_LOC_ID = :agencyId
      AND OB.BOOKING_SEQ = :bookingSeq
      AND OHR.FINDING_CODE = :hearingFinding
      AND OOS.CREATE_DATETIME > :threeMonthsAgo
    GROUP BY OB.OFFENDER_BOOK_ID
    ORDER BY PROVEN_ADJUDICATIONS DESC
) PROVEN_ADJUDICATIONS_SELECT ON IEP_DETAILS.OFFENDER_BOOK_ID = PROVEN_ADJUDICATIONS_SELECT.OFFENDER_BOOK_ID
         FULL OUTER JOIN (SELECT OB.OFFENDER_BOOK_ID,
                          ...
                          WHERE OB.AGY_LOC_ID = :agencyId
                            AND OB.BOOKING_SEQ = :bookingSeq
) OFFENDER_DETAILS ON IEP_DETAILS.OFFENDER_BOOK_ID = OFFENDER_DETAILS.OFFENDER_BOOK_ID
...
ORDER BY IEP_DETAILS.IEP_TIME DESC, POS_NEG_IEPS.NEGATIVE_IEPS DESC
}
