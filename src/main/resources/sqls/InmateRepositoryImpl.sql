...
FIND_PHYSICAL_MARKS_BY_BOOKING {
  SELECT (SELECT DESCRIPTION FROM REFERENCE_CODES WHERE CODE = M.MARK_TYPE AND DOMAIN='MARK_TYPE') AS TYPE,
         ...
         (SELECT I.OFFENDER_IMAGE_ID
          FROM OFFENDER_IMAGES I
          WHERE OFFENDER_IMAGE_ID = (SELECT MAX(MI.OFFENDER_IMAGE_ID)
                                      FROM OFFENDER_IMAGES MI
                                      WHERE B.OFFENDER_BOOK_ID = MI.OFFENDER_BOOK_ID
                                      AND MI.ACTIVE_FLAG = 'Y'
                                      AND M.MARK_TYPE = MI.IMAGE_VIEW_TYPE
                                      AND M.BODY_PART_CODE = MI.ORIENTATION_TYPE
                                      AND M.ID_MARK_SEQ = MI.IMAGE_OBJECT_ID)
         ) AS IMAGE_ID
