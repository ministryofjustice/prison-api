...
FIND_LOCAL_ADMINISTRATOR_USERS_BY_CASELOAD {
  SELECT SM.STAFF_ID,
       SUA.USERNAME,
       SM.FIRST_NAME,
       SM.LAST_NAME
  FROM STAFF_USER_ACCOUNTS SUA
       INNER JOIN STAFF_MEMBERS SM ON SUA.STAFF_ID = SM.STAFF_ID
       INNER JOIN LAA_GENERAL_USERS LAA ON SUA.USERNAME = LAA.USERNAME AND ACTIVE_FLAG = :activeFlag
  WHERE LAA.LOCAL_AUTHORITY_CODE = :caseloadId
