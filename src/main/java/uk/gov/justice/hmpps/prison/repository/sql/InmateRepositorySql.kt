...
enum class InmateRepositorySql(val sql: String) {
  FIND_INMATE_DETAIL(
    """
        ...
        AND OI.ORIENTATION_TYPE = 'FRONT') AS FACE_IMAGE_ID,
        O.BIRTH_DATE,
        O.BIRTH_PLACE,
  ...
  FIND_ALL_INMATES(
    """
        ...
        FROM (
             SELECT OIST.* FROM OFFENDER_IMPRISON_STATUSES OIST WHERE OIST.IMPRISON_STATUS_SEQ IN 
        ...
        WHERE OIS.OFFENDER_BOOK_ID = OB.OFFENDER_BOOK_ID AND
            OB.ACTIVE_FLAG = 'Y' AND OB.BOOKING_SEQ = 1
    """
  ),
  ...
  FIND_IDENTIFIER_RECORDS_BY_TYPE_AND_VALUE(
    """
        SELECT O.OFFENDER_ID_DISPLAY OFFENDER_NO,
        OB.OFFENDER_BOOK_ID BOOKING_ID,
        IDENTIFIER_TYPE "TYPE",
        OI.IDENTIFIER "VALUE",
        ISSUED_AUTHORITY_TEXT,
        ...
        and OI.IDENTIFIER = :identifierValue
        and OI.OFFENDER_ID_SEQ = (SELECT MAX(OFFENDER_ID_SEQ)
                FROM OFFENDER_IDENTIFIERS OI2
                WHERE OI2.OFFENDER_ID = OI.OFFENDER_ID
                AND OI2.IDENTIFIER_TYPE = OI.IDENTIFIER_TYPE )
