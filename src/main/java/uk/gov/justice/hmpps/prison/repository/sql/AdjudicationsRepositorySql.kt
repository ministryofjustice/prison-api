...
enum class AdjudicationsRepositorySql(val sql: String) {
  FIND_AWARDS(
    """
        ...
        S.EFFECTIVE_DATE         EFFECTIVE_DATE,OIC_INCIDENT_ID
        S.STATUS,
        RC2.DESCRIPTION          STATUS_DESCRIPTION,
        S.OIC_HEARING_ID         HEARING_ID,
        S.RESULT_SEQ             HEARING_SEQUENCE,
        H.OIC_INCIDENT_ID        ADJUDICATION_NUMBER,
        AIP.AGENCY_INCIDENT_ID   REPORT_ID
        FROM OFFENDER_OIC_SANCTIONS S
        INNER JOIN OIC_HEARING_RESULTS HR ON S.OIC_HEARING_ID = HR.OIC_HEARING_ID AND S.RESULT_SEQ = HR.RESULT_SEQ
        INNER JOIN OIC_HEARINGS H ON S.OIC_HEARING_ID = H.OIC_HEARING_ID
        INNER JOIN AGENCY_INCIDENT_PARTIES AIP ON AIP.OIC_INCIDENT_ID = H.OIC_INCIDENT_ID
