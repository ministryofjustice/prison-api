...
enum class MovementsRepositorySql(val sql: String) {
  ...
  GET_MOVEMENTS_BY_OFFENDER(
    """
        ...
        AND OFFENDERS.OFFENDER_ID_DISPLAY in (:offenderNumber)
    """,
  ),
  ...
  GET_OFFENDER_MOVEMENTS_IN(
    """
        ...
        ADDR.PREMISE AS FROM_ADDR_PREMISE,
        ADDR.STREET AS FROM_ADDR_STREET,
        ADDR.LOCALITY AS FROM_ADDR_LOCALITY,
        NVL(RC3.DESCRIPTION,RC5.DESCRIPTION) AS FROM_ADDR_CITY,
        ADDR.POSTAL_CODE AS FROM_ADDR_POSTAL_CODE
        FROM OFFENDER_EXTERNAL_MOVEMENTS OEM
        INNER JOIN OFFENDER_BOOKINGS OB      ON OB.OFFENDER_BOOK_ID = OEM.OFFENDER_BOOK_ID AND OB.BOOKING_SEQ = 1
        INNER JOIN OFFENDERS O               ON O.OFFENDER_ID = OB.OFFENDER_ID
        LEFT JOIN REFERENCE_CODES RC2 ON RC2.CODE = OEM.MOVEMENT_REASON_CODE AND RC2.DOMAIN = 'MOVE_RSN'
        ...
        LEFT JOIN REFERENCE_CODES RC3 ON RC3.CODE = OEM.FROM_CITY AND RC3.DOMAIN = 'CITY'
        LEFT JOIN ADDRESSES ADDR ON ADDR.ADDRESS_ID = OEM.FROM_ADDRESS_ID
        LEFT JOIN REFERENCE_CODES RC5 ON RC5.CODE = ADDR.CITY_CODE AND RC5.DOMAIN = 'CITY'
        WHERE
  ...
  GET_OFFENDERS_OUT_TODAY(
    """
        ...
        RC2.DESCRIPTION AS MOVEMENT_REASON_DESCRIPTION,
        OEM.MOVEMENT_TIME,
        ADDR.PREMISE AS TO_ADDR_PREMISE,
        ADDR.STREET AS TO_ADDR_STREET,
        ADDR.LOCALITY AS TO_ADDR_LOCALITY,
        NVL(RC4.DESCRIPTION,RC5.DESCRIPTION) AS TO_ADDR_CITY,
        ADDR.POSTAL_CODE AS TO_ADDR_POSTAL_CODE,
        OEM.MOVEMENT_SEQ AS MOVEMENT_SEQUENCE
        FROM OFFENDER_EXTERNAL_MOVEMENTS OEM
        INNER JOIN OFFENDER_BOOKINGS OB    ON OB.OFFENDER_BOOK_ID = OEM.OFFENDER_BOOK_ID AND OB.BOOKING_SEQ = 1
        INNER JOIN OFFENDERS               ON OFFENDERS.OFFENDER_ID = OB.OFFENDER_ID
        LEFT JOIN REFERENCE_CODES RC2 ON RC2.CODE = OEM.MOVEMENT_REASON_CODE AND RC2.DOMAIN = 'MOVE_RSN'
        LEFT JOIN REFERENCE_CODES RC4 ON RC4.CODE = OEM.TO_CITY AND RC4.DOMAIN = 'CITY'
        LEFT JOIN ADDRESSES ADDR ON ADDR.ADDRESS_ID = OEM.TO_ADDRESS_ID
        LEFT JOIN REFERENCE_CODES RC5 ON RC5.CODE = ADDR.CITY_CODE AND RC5.DOMAIN = 'CITY'
        WHERE
  ...
  GET_COURT_EVENTS_BY_AGENCY_AND_TIME_PERIOD(
    """
        ...
        AND (OB.AGY_LOC_ID  = :agencyFrom OR CE.AGY_LOC_ID = :agencyTo)
    """,
  ),
  ...
  GET_OFFENDER_INDIVIDUAL_SCHEDULES_BY_DATE(
    """
        ...
        AND (OIS.AGY_LOC_ID = :agencyFrom OR OIS.TO_AGY_LOC_ID = :agencyTo)
    """,
  ),
  ...
  GET_OFFENDER_RELEASES_BY_AGENCY_AND_DATE(
    """
        ...
        WHERE ORD.RELEASE_DATE = :date
        AND OB.AGY_LOC_ID = :agencyFrom
    """,
  ),

  GET_LATEST_ARRIVAL_DATE(
    """
          WITH MAX_MOVEMENT_SEQ AS (
              SELECT OEM2.OFFENDER_BOOK_ID, MAX(OEM2.MOVEMENT_SEQ) AS MAX_SEQ
              FROM OFFENDER_EXTERNAL_MOVEMENTS OEM2
              WHERE OEM2.MOVEMENT_TYPE = 'ADM'
              GROUP BY OEM2.OFFENDER_BOOK_ID
          )
          SELECT OEM.MOVEMENT_DATE
          FROM OFFENDER_EXTERNAL_MOVEMENTS OEM
          INNER JOIN MAX_MOVEMENT_SEQ MMS ON OEM.OFFENDER_BOOK_ID = MMS.OFFENDER_BOOK_ID
          AND OEM.MOVEMENT_SEQ = MMS.MAX_SEQ
          INNER JOIN OFFENDER_BOOKINGS OB ON OB.OFFENDER_BOOK_ID = OEM.OFFENDER_BOOK_ID
          INNER JOIN OFFENDERS ON OFFENDERS.OFFENDER_ID = OB.OFFENDER_ID
          WHERE OFFENDERS.OFFENDER_ID_DISPLAY = :offenderNumber
          AND OEM.MOVEMENT_TYPE = 'ADM'
          AND OB.BOOKING_SEQ = 1
        """,
  ),

  GET_LATEST_ARRIVAL_DATES(
    """
          WITH MAX_MOVEMENT_SEQ AS (
              SELECT OEM2.OFFENDER_BOOK_ID, MAX(OEM2.MOVEMENT_SEQ) AS MAX_SEQ
              FROM OFFENDER_EXTERNAL_MOVEMENTS OEM2
          ...
          SELECT
          O.OFFENDER_ID_DISPLAY  AS OFFENDER_NO,
          OEM.MOVEMENT_DATE      AS LATEST_ARRIVAL_DATE
          FROM OFFENDER_EXTERNAL_MOVEMENTS OEM
          INNER JOIN MAX_MOVEMENT_SEQ MMS ON OEM.OFFENDER_BOOK_ID = MMS.OFFENDER_BOOK_ID
          AND OEM.MOVEMENT_SEQ = MMS.MAX_SEQ
          INNER JOIN OFFENDER_BOOKINGS OB ON OB.OFFENDER_BOOK_ID = OEM.OFFENDER_BOOK_ID
          INNER JOIN OFFENDERS O ON O.OFFENDER_ID = OB.OFFENDER_ID
          WHERE O.OFFENDER_ID_DISPLAY IN (:offenderNumbers)
          AND OEM.MOVEMENT_TYPE = 'ADM'
          AND OB.BOOKING_SEQ = 1
        """,
  ),
