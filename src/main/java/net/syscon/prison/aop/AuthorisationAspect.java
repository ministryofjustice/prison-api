...
import net.syscon.prison.service.support.AgencyRequest;
...
public class AuthorisationAspect {
    ...
    @Pointcut("@annotation(net.syscon.prison.security.VerifyOffenderAccess) && execution(* *(String,..)) && args(offenderNo,..)")
    public void verifyOffenderAccessPointcut(final String offenderNo) {
        // no code needed - pointcut definition
    }

    @Pointcut("@annotation(net.syscon.prison.security.VerifyBookingAccess) && execution(* *(Long,..)) && args(bookingId,..)")
    public void verifyBookingAccessPointcut(final Long bookingId) {
        // no code needed - pointcut definition
    }

    @Pointcut("@annotation(net.syscon.prison.security.VerifyAgencyAccess) && execution(* *(String,..)) && args(agencyId,..)")
    public void verifyAgencyAccessPointcut(final String agencyId) {
        // no code needed - pointcut definition
    }

    @Pointcut("@annotation(net.syscon.prison.security.VerifyAgencyAccess) && args(net.syscon.prison.service.support.AgencyRequest,..) && args(request,..)")
