...
public class CaseNoteResourceImpl implements CaseNoteResource {
    ...
    @Override
    public GetCaseNoteUsageSummaryResponse getCaseNoteUsageSummary(List<String> offenderNo, Integer staffId, String agencyId, Integer numMonths, LocalDate fromDate, LocalDate toDate, String type, String subType) {
        List<CaseNoteUsage> caseNoteUsage = caseNoteService.getCaseNoteUsage(type, subType, offenderNo, staffId, agencyId, fromDate, toDate, ObjectUtils.defaultIfNull(numMonths, ObjectUtils.defaultIfNull(numMonths, 1)));
    ...
    public GetCaseNoteUsageSummaryByPostResponse getCaseNoteUsageSummaryByPost(CaseNoteUsageRequest request) {
        List<CaseNoteUsage> caseNoteUsage = caseNoteService.getCaseNoteUsage(request.getType(), request.getSubType(), request.getOffenderNos(), request.getStaffId(), request.getAgencyId(), request.getFromDate(), request.getToDate(), ObjectUtils.defaultIfNull(request.getNumMonths(), 1));
