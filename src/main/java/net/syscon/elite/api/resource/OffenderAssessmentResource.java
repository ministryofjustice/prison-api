...
public interface OffenderAssessmentResource {
    ...
    List<Assessment>  getOffenderAssessmentsAssessmentCode(@ApiParam(value = "Assessment Type Code", required = true) @PathVariable("assessmentCode") String assessmentCode,
                                                                                      @ApiParam(value = "The required offender numbers", required = true) @RequestParam("offenderNo") List<String> offenderNo,
                                                                                      @ApiParam(value = "Returns only the latest assessment for the current sentence if true, otherwise all assessments for all previous sentences are included", defaultValue = "true") @RequestParam("latestOnly") Boolean latestOnly,
                                                                                      @ApiParam(value = "Returns only active assessments if true, otherwise assessments with any status are included", defaultValue = "true") @RequestParam("activeOnly") Boolean activeOnly);
    ...
    List<Assessment>  postOffenderAssessmentsCsraList(@ApiParam(value = "The required offender numbers (mandatory)", required = true) List<String> body);
    ...
    List<OffenderCategorise> getOffenderCategorisations(@ApiParam(value = "Prison id", required = true) @PathVariable("agencyId") String agencyId,
                                                        ...
                                                        @ApiParam(value = "For type CATEGORISED: The past date from which categorisations are returned.<br />" +
                                                                ...
                                                                "For type UNCATEGORISED: Ignored; do not set this parameter.") @RequestParam("date") LocalDate date);
    ...
    List<OffenderCategorise> getOffenderCategorisations(@ApiParam(value = "Prison id", required = true) @PathVariable("agencyId") String agencyId,
                                                        ...
                                                        @ApiParam(value = "Only get the latest category for each booking", defaultValue = "true") @RequestParam("latestOnly") Boolean latestOnly);
    ...
    ResponseEntity<Void> createCategorisation(@ApiParam(value = "Categorisation details", required = true) @Valid CategorisationDetail body);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 400, message = "Invalid request - e.g. category does not exist.", response = ErrorResponse.class)})
    ResponseEntity<Void> updateCategorisation(@ApiParam(value = "Categorisation details", required = true) @Valid CategorisationUpdateDetail body);
    ...
    ResponseEntity<Void> approveCategorisation(@ApiParam(value = "Approval details", required = true) @Valid CategoryApprovalDetail body);
    ...
    ResponseEntity<Void> rejectCategorisation(@ApiParam(value = "Rejection details", required = true) @Valid CategoryRejectionDetail body);
    ...
    ResponseEntity<Void> setCategorisationInactive(
            ...
            @ApiParam(value = "Indicates which categorisation statuses to set." +
                    ...
                    "<li>PENDING: set all pending (i.e. awaiting approval) categorisations inactive,</li>") @RequestParam("status") String status);
