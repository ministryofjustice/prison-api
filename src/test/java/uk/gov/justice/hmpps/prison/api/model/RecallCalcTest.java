...
class RecallCalcTest {
    ...
    void shouldReturnTrueWhenSentenceTypeIsARecallCode(final String sentenceType) {
        var sentenceTerm = OffenderSentenceTerms.builder()
            .sentenceType(sentenceType)
            .build();

        boolean result = RecallCalc.calculate(
            1L,
            LegalStatus.CONVICTED_UNSENTENCED,
            Collections.emptyList(),
            List.of(sentenceTerm)
        );
