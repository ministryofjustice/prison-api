...
class OffenderResourceImplIntTest_updateEthnicity : ResourceTest() {
  ...
  inner class Authorisation {
    ...
    fun `returns 401 without an auth token`() {
      webTestClient.put()
        .uri("api/offenders/A1234AL/ethnicity")
        .header("Content-Type", APPLICATION_JSON_VALUE)
        .bodyValue(VALID_ETHNICITY_UPDATE)
        .exchange()
  ...
  open inner class HappyPath {
    ...
    open fun `should update the ethnicity`() {
      webTestClient.put()
        .uri("api/offenders/A1234AL/ethnicity")
        .headers(setClientAuthorisation(listOf("ROLE_PRISON_API__PRISONER_PROFILE__RW")))
        .header("Content-Type", APPLICATION_JSON_VALUE)
      ...
      assertThat(offenderRepository.findById(-1012L).get().ethnicity.code).isEqualTo("M3")
    }
    ...
    open fun `should allow the birth country to be updated to null`() {
      webTestClient.put()
        ...
        .headers(setClientAuthorisation(listOf("ROLE_PRISON_API__PRISONER_PROFILE__RW")))
        .header("Content-Type", APPLICATION_JSON_VALUE)
        .bodyValue(NULL_ETHNICITY_UPDATE)
        .exchange()
        .expectStatus().isNoContent

      assertThat(offenderRepository.findById(-1012L).get().ethnicity).isNull()
    }
  }
  ...
  inner class ErrorConditions {
    ...
    fun shouldReturn404WhenOffenderDoesNotExist() {
      webTestClient.put()
        .uri("api/offenders/AAA444/ethnicity")
        .headers(setClientAuthorisation(listOf("ROLE_PRISON_API__PRISONER_PROFILE__RW")))
        .header("Content-Type", APPLICATION_JSON_VALUE)
        .bodyValue(VALID_ETHNICITY_UPDATE)
        .exchange()
    ...
    fun shouldReturn404WhenOffenderHasNoBooking() {
      webTestClient.put()
        .uri("api/offenders/A9880GH/ethnicity")
        .headers(setClientAuthorisation(listOf("ROLE_PRISON_API__PRISONER_PROFILE__RW")))
        .header("Content-Type", APPLICATION_JSON_VALUE)
        .bodyValue(VALID_ETHNICITY_UPDATE)
        .exchange()
    ...
    fun `returns status 423 (locked) when database row lock times out`() {
      doThrow(DatabaseRowLockedException("developer message"))
        .whenever(prisonerProfileUpdateService).updateEthnicityOfCurrentAlias(anyString(), anyString())
  ...
  private companion object {
    const val VALID_ETHNICITY_UPDATE =
      // language=json
      """
        {
          "ethnicity": "M3"
    ...
    const val NULL_ETHNICITY_UPDATE =
      // language=json
      """
        {
          "ethnicity": null
