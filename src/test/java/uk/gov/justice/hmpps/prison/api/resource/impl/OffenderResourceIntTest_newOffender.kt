...
class OffenderResourceIntTest_newOffender : ResourceTest() {
  ...
  fun expectBadRequest(body: Any, allowNameDuplicate: Boolean = false): WebTestClient.BodyContentSpec = webTestClient.post().uri("/api/offenders?allowNameDuplicate={allowNameDuplicate}", allowNameDuplicate).headers(setAuthorisation(listOf("ROLE_BOOKING_CREATE")))
  ...
  fun expectNotFoundRequest(body: Any): WebTestClient.BodyContentSpec = webTestClient.post().uri("/api/offenders").headers(setAuthorisation(listOf("ROLE_BOOKING_CREATE")))
    .header("Content-Type", MediaType.APPLICATION_JSON_VALUE).bodyValue(body).accept(MediaType.APPLICATION_JSON)
    .exchange().expectStatus().isNotFound.expectBody().jsonPath("$.status").isEqualTo(404)

  fun expectSuccess(body: Any, allowNameDuplicate: Boolean = false) = webTestClient.post().uri("/api/offenders?allowNameDuplicate={allowNameDuplicate}", allowNameDuplicate).headers(setAuthorisation(listOf("ROLE_BOOKING_CREATE")))
    .header("Content-Type", MediaType.APPLICATION_JSON_VALUE).bodyValue(body).accept(MediaType.APPLICATION_JSON)
