...
import uk.gov.justice.hmpps.prison.repository.jpa.model.StaffUserAccount.builder
...
class StaffUserAccountRepositoryTest(
  @Autowired private val repository: StaffUserAccountRepository,
) {
  @Test
  fun getUserDetails() {
    val user = repository.findById("ITAG_USER")

    assertThat(user).get().usingRecursiveComparison()
      .ignoringFields("createUserId", "createDatetime", "caseloads", "roles", "staff")
      .isEqualTo(staffUserAccountRepositoryResponse())
  }

  fun staffUserAccountRepositoryResponse(): StaffUserAccount = builder()
    .username("ITAG_USER")
    .staff(
      Staff.builder()
        .staffId(-2L)
    ...
    .roles(
      listOf(
        UserCaseloadRole.builder()
          .id(
            UserCaseloadRoleIdentity.builder()
