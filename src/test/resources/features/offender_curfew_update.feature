Feature: Updating Offender Curfew state
  ...
  Scenario Outline:  Update the 'Approval status' for a booking's current (latest) curfew
    Given a user has a token name of "<tokenName>"
    When that user requests an update of the HDC approval status of the latest Offender Curfew for booking "<bookingId>" to "<approvalStatus>" at "<approvalStatusDate>"
    Then the response HTTP status should be "<httpStatus>"

    Examples:
      | tokenName              | bookingId | approvalStatus | approvalStatusDate | httpStatus |
      ...
      | SYSTEM_USER_READ_WRITE | -35       |                |         2018-01-31 |        404 |
