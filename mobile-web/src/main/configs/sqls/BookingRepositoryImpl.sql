...
SELECT * FROM (SELECT 
  ...
  FROM OFFENDER_VISITS VISIT
    ...
    LEFT JOIN REFERENCE_CODES RC1 ON RC1.DOMAIN = 'RELATIONSHIP' AND OCP.RELATIONSHIP_TYPE = RC1.CODE
...
FIND_BOOKINGS_BY_PERSON_CONTACT {
  ...
  WHERE PI.IDENTIFIER = :identifier
        AND    (OCP.RELATIONSHIP_TYPE = :relationshipType OR :relationshipType is null)
  AND OB.ACTIVE_FLAG = 'Y'
}
...
FROM OFFENDER_BOOKINGS OB
  ...
  LEFT JOIN AGENCY_INTERNAL_LOCATIONS AIL ON OB.LIVING_UNIT_ID = AIL.INTERNAL_LOCATION_ID
WHERE (OCP.RELATIONSHIP_TYPE = :relationshipType OR :relationshipType is null)
      AND OB.ACTIVE_FLAG = 'Y'
}