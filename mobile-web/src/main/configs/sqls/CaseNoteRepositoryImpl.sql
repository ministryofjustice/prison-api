...
GET_CASE_NOTE_TYPES_WITH_SUB_TYPES_BY_CASELOAD_TYPE {
	SELECT DISTINCT W.WORK_TYPE CODE,
	        RC.DOMAIN,
	        RC.DESCRIPTION,
		      RC.PARENT_DOMAIN PARENT_DOMAIN_ID,
		      RC.PARENT_CODE,
	...
	FROM WORKS W
	  INNER JOIN REFERENCE_CODES RC ON RC.CODE = W.WORK_TYPE AND RC.ACTIVE_FLAG = 'Y'
	  INNER JOIN REFERENCE_CODES RCSUB ON RCSUB.DOMAIN = 'TASK_SUBTYPE' AND RC.ACTIVE_FLAG = 'Y'
...
GET_USED_CASE_NOTE_TYPES_WITH_SUB_TYPES {
  SELECT DISTINCT RC1.CODE,
    ...
    RC2.ACTIVE_FLAG SUB_ACTIVE_FLAG
  FROM OFFENDER_CASE_NOTES OCN
    INNER JOIN REFERENCE_CODES RC1 ON RC1.CODE = OCN.CASE_NOTE_TYPE AND RC1.DOMAIN = 'TASK_TYPE'
    INNER JOIN REFERENCE_CODES RC2 ON RC2.CODE = OCN.CASE_NOTE_SUB_TYPE AND RC2.DOMAIN = 'TASK_SUBTYPE'
  ORDER BY RC1.CODE, RC2.CODE
}