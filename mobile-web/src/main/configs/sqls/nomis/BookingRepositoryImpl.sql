...
GET_BOOKING_ACTIVITIES {
  ...
  FROM OFFENDER_PROGRAM_PROFILES OPP
    ...
    INNER JOIN COURSE_SCHEDULES CS ON OPP.CRS_ACTY_ID = CS.CRS_ACTY_ID
      ...
      AND COALESCE(OPP.OFFENDER_END_DATE, CA.SCHEDULE_END_DATE, CS.SCHEDULE_DATE) >= CS.SCHEDULE_DATE
  ...
  WHERE OPP.OFFENDER_BOOK_ID = :bookingId
    ...
    AND (TO_CHAR(CS.SCHEDULE_DATE, 'DY'), CS.SLOT_CATEGORY_CODE) NOT IN
	    (SELECT OE.EXCLUDE_DAY,
	            COALESCE(OE.SLOT_CATEGORY_CODE, CS.SLOT_CATEGORY_CODE)
