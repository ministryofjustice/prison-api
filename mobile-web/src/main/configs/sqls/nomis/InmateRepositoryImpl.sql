...
FIND_ALL_INMATES {
      SELECT
        ...
        NVL((SELECT initcap(substr(SYS_CONNECT_BY_PATH(A.INTERNAL_LOCATION_TYPE || ' ' || A.INTERNAL_LOCATION_CODE, '|'),2))
             FROM AGENCY_INTERNAL_LOCATIONS A
             WHERE A.INTERNAL_LOCATION_TYPE = :locationTypeRoot
             START WITH A.INTERNAL_LOCATION_ID = AIL.INTERNAL_LOCATION_ID
             CONNECT BY PRIOR A.PARENT_INTERNAL_LOCATION_ID  = A.INTERNAL_LOCATION_ID), AIL.DESCRIPTION) as LIVING_UNIT_DESC,
        (
          SELECT OI.OFFENDER_IMAGE_ID
          FROM OFFENDER_IMAGES OI
          WHERE OI.ACTIVE_FLAG = 'Y'
...
FIND_PRISONERS {
  SELECT
    ...
    CASE WHEN CAST(ist.band_code AS NUMBER) <= 8
      THEN 'Convicted'
    WHEN CAST(ist.band_code AS NUMBER) > 8
      THEN 'Remand'
