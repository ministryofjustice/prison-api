...
FIND_ALL_INMATES {
      SELECT
        ...
        B.LIVING_UNIT_ID,
        NVL((SELECT initcap(substr(SYS_CONNECT_BY_PATH(A.INTERNAL_LOCATION_TYPE || ' ' || A.INTERNAL_LOCATION_CODE, '/'),2))
             FROM AGENCY_INTERNAL_LOCATIONS A
             WHERE A.INTERNAL_LOCATION_ID = AIL.INTERNAL_LOCATION_ID
             START WITH A.INTERNAL_LOCATION_TYPE = :locationTypeRoot
             CONNECT BY PRIOR A.INTERNAL_LOCATION_ID = A.PARENT_INTERNAL_LOCATION_ID), AIL.DESCRIPTION) as LIVING_UNIT_DESC,
        (
          SELECT OI.OFFENDER_IMAGE_ID
          FROM OFFENDER_IMAGES OI
          WHERE OI.ACTIVE_FLAG = 'Y'
...
FIND_PHYSICAL_CHARACTERISTICS_BY_BOOKING {
  ...
  from OFFENDER_PROFILE_DETAILS P
    ...
    JOIN PROFILE_TYPES PT ON PT.PROFILE_TYPE = P.PROFILE_TYPE AND PROFILE_CATEGORY IN ('PA')
