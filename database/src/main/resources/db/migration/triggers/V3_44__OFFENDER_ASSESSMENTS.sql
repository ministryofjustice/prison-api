CREATE OR REPLACE TRIGGER "OFFENDER_ASSESSMENTS_TA"
BEFORE INSERT OR UPDATE OR DELETE ON OFFENDER_ASSESSMENTS
...
CREATE OR REPLACE TRIGGER "OFFENDER_ASSESSMENTS_T1"
BEFORE INSERT OR UPDATE ON offender_assessments
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
  DECLARE

    lv_csa assessments.cell_sharing_alert_flag%TYPE;
  ...
  BEGIN
    ...
    /*
    ============================================================
       Version Number = 2.2  Date Modified = 06-Jan-2011
    ============================================================
      MODIFICATION HISTORY
       Person    Date      version      Comments
    -----------  --------- -----------  -------------------------------
         Kumar   06/01/2011  2.2        #18219: Replaced approved_sup_level_type with review_sup_level_type
         Graham  21/12/2007  2.1        #7775: Code added for Merge, fix versioning
           Neil  03/03/2006  2.0        Initial version
    */
    lv_csa := tag_assessment.chk_assessment_csa
    (:NEW.assessment_type_id,
     NVL(:NEW.review_sup_level_type, --Kumar #18219
         NVL(:NEW.overrided_sup_level_type,
             :NEW.calc_sup_level_type)));

    IF lv_csa IS NOT NULL
    THEN
      tag_assessment.update_bkg_csa
