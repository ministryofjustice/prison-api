...
CREATE OR REPLACE TRIGGER "USER_ACCESSIBLE_CASELOADS_TA"
BEFORE INSERT OR UPDATE OR DELETE ON USER_ACCESSIBLE_CASELOADS
...
CREATE OR REPLACE TRIGGER "USER_ACCESSIBLE_CASELOADS_T1"
AFTER INSERT OR DELETE
  ON user_accessible_caseloads
REFERENCING new AS new old AS old
FOR EACH ROW
  DECLARE
    /******************************************************************************
       Name:  user_accessible_caseloads_t1
       PURPOSE: This trigger initiates a job to grant or revoke a proxy
                user

       Revisions:
       Ver        Date          Author           Description
       ---------  ------------  ---------------  ------------------------------------
       1.0        02/10/2017    Paul Morris      Initial version
    ******************************************************************************/
  ...
  BEGIN
    IF INSERTING
    THEN
      IF :new.caseload_id = 'NWEB'
      THEN
        dbms_job.submit(job       => v_job,
                        what      => 'api_proxy_admin.grant_proxy(''' || :new.username || ''');',
                        next_date => sysdate);
      END IF;
    ELSIF DELETING
      THEN
        IF :old.caseload_id = 'NWEB'
        THEN
          dbms_job.submit(job       => v_job,
                          what      => 'api_proxy_admin.revoke_proxy(''' || :old.username || ''');',
                          next_date => sysdate);
        END IF;
    END IF;
  END;
