...
CREATE OR REPLACE TRIGGER "COURSE_SCHEDULES_TA"
BEFORE INSERT OR UPDATE OR DELETE ON COURSE_SCHEDULES
...
CREATE OR REPLACE TRIGGER "COURSE_SCHEDULES_T1"
BEFORE
INSERT
  ON course_schedules
...
FOR EACH ROW
  ...
  BEGIN
    ...
    -----------  --------- -----------  -------------------------------
       ...
       David Ng    17/03/2006  2.0        Assign proper sequence
       Neil        22/12/2005  1.1        Initial version
    */


    IF (INSERTING AND :New.Crs_Sch_ID IS NULL)
    THEN
      SELECT CRS_sch_ID.NEXTVAL
      INTO l_crs_sch_id
      FROM DUAL;
      :new.crs_sch_id := l_crs_sch_id;
    END IF;
  END;
