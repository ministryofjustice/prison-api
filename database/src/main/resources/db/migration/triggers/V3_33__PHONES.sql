CREATE OR REPLACE TRIGGER "PHONES_T2"
AFTER
...
FOR EACH ROW
  ...
  BEGIN
    ...
    */
    IF INSERTING
    THEN
      IF :NEW.owner_class = 'ADDR'
      THEN
        lv_agy_loc_id := oumagyht.get_address_owner_code (:NEW.owner_id);
      END IF;
    ELSIF UPDATING OR DELETING
      THEN
        lv_agy_loc_id := oumagyht.get_address_owner_code (:OLD.owner_id);
    END IF;

    IF lv_agy_loc_id IS NULL
    THEN
      IF INSERTING
      THEN
    ...
    IF lv_agy_loc_id IS NOT NULL THEN
      IF oumagyht.check_changed (:OLD.phone_type, :NEW.phone_type)
      THEN
        oumagyht.insert_into_agy_loc_amendments (lv_agy_loc_id, 'PHONE_TYPE', :OLD.phone_type, :NEW.phone_type, 'PHONE_USAGE', 'REF_CODE');
      END IF;

      IF oumagyht.check_changed (:OLD.phone_no, :NEW.phone_no)
      THEN
        oumagyht.insert_into_agy_loc_amendments (lv_agy_loc_id, 'PHONE_NO', :OLD.phone_no, :NEW.phone_no, '', '');
      END IF;

      IF oumagyht.check_changed (:OLD.ext_no, :NEW.ext_no)
      THEN
        oumagyht.insert_into_agy_loc_amendments (lv_agy_loc_id, 'EXT_NO', :OLD.ext_no, :NEW.ext_no, '', '');
      END IF;
    END IF;
  END;

/
ALTER TRIGGER "PHONES_T2" ENABLE;


CREATE OR REPLACE TRIGGER "PHONES_T1"
BEFORE
INSERT OR UPDATE
  ON phones
REFERENCING NEW AS NEW OLD AS OLD
    ...
    IF :new.owner_class = 'OFF' THEN
      ...
      THEN
        raise_application_error(
            -20001,
            'Cannot INSERT phone record due to missing offender record.'
        );
      END IF;
    ELSIF  :new.owner_class = 'ADDR' THEN
      SELECT count(*) into v_numrows
      FROM   addresses
      ...
      END IF;
    ELSIF  :new.owner_class = 'PER' THEN
      SELECT count(*) into v_numrows
      FROM   persons
      ...
      THEN
        raise_application_error(
            -20001,
            'Cannot INSERT phone record due to missing person records.'
        );
      END IF;
    ELSIF :new.owner_class = 'CORP' THEN
      SELECT count(*) into v_numrows
      FROM   corporates
      ...
      THEN
        raise_application_error(
            -20001,
            'Cannot INSERT phone record due to missing corporate record.'
        );
      END IF;
    ELSIF :new.owner_class = 'STF' THEN
      SELECT count(*) into v_numrows
      FROM   staff_members
      ...
      THEN
        raise_application_error(
            -20001,
            'Cannot INSERT phone record due to missing staff record.'
        );
      END IF;
    ELSIF :new.owner_class = 'AGY' THEN
      SELECT count(*) into v_numrows
      ...
      THEN
        raise_application_error(
            -20001,
            'Cannot INSERT phone record due to missing agency location record.'
        );
      END IF;
    ELSIF :new.owner_class = 'OFF_EDU' THEN
      SELECT count(*) into v_numrows
      ...
      END IF;
    ELSIF :new.owner_class = 'OFF_EMP' THEN
      SELECT count(*) into v_numrows
      ...
      END IF;
    ELSIF :new.owner_class = 'PER_EMP' THEN
      SELECT count(*) into v_numrows
...
CREATE OR REPLACE TRIGGER "PHONES_TA"
BEFORE INSERT OR UPDATE OR DELETE ON PHONES
